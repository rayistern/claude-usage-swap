...
**1. Spreading load is the watchdog's job — the daemon cannot do it.** The daemon moves whole slots, one at a time, to whichever account scores best right now; it cannot see that one slot holds six heavy panes, that a move costs a context rebuild, or that it just moved three walled lanes onto the same fresh account. The watchdog can. So when there is more than one account with room, the watchdog actively places for balance:
   ...
   - **Split a slot that is bigger than any single account.** A slot whose combined burn walls a fresh account in under ~30 minutes is too big to place anywhere; relaunch some of its panes into their own lanes. Mechanics learned 2026-09-18: `cus slot move <idle-slot> <acct>` to give a free lane an account (claims a family), then relaunch the pane with `CLAUDE_CONFIG_DIR=/home/rayi/claude-accounts/<slot> claude --resume <session-id> --dangerously-skip-permissions` in its own cwd. **Do NOT use `cus launch <acct> --lane <slot>` for this** — it refuses a lane whose account is already live elsewhere even when the lane holds its own family, and by the time it refuses, the pane's old claude is already gone. A pane whose tmux process IS claude (no shell underneath) must be restarted with `tmux respawn-pane -k -t <pane> -c <cwd> "bash -l"` first, or exiting claude closes the pane. A fresh slot shows the trust-folder prompt the first time it opens a project — the owner authorized answering "Yes, I trust this folder" for panes the watchdog relaunched (2026-09-18). Send a signed resume message afterwards: a `--resume` does not continue on its own.
