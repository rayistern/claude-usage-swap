...
>
> **So:** state what you see, say what you do NOT know (what their work needs), and **ask**. *"rayi1 is at 66% with 53 minutes left — what's in front of you, and does it fit?"* invites a real answer. Offer to do the arithmetic FOR them if they describe the payload, instead of handing over a rate and leaving them to guess. **Name which axis is scarce**: Opus and Sonnet seats cost 5h but zero Fable, so "expensive" depends entirely on which wall is near. Say plainly that the message is information they may ignore, not a constraint. And if you got it wrong, send the correction — leaving a peer over-constrained is worse than the original message.
>
