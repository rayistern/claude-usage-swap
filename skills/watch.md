...
- **Model split: the watchdog LOOP runs on Opus; real WORK it spawns runs on Fable (owner, 2026-09-18: *"opus is supposed to be for the basic watchdog, fable for when you end up doing work"*).** The standing tick — read panes, poll accounts, move lanes, report — is Opus on a `standard`-pool slot, which burns **zero Fable**. That is what lets the dedicated host be a Fable-exhausted account the fleet can't use for premium lanes (rayi2 at Fable 100% is a *fine* host for an Opus watchdog, and a wall for a Fable one). Anything the watchdog delegates that is actual work — building a feature, fixing review findings, drafting a rule — goes to a subagent with `model: fable`. Code-review seats stay Opus + Sonnet per the dual-review gate. **If the watchdog session itself is ever switched to Fable, its host choice is no longer valid**: re-check the host's Fable weekly immediately, because a Fable-dead host that was correct for Opus will wall a Fable watchdog.
