...
- **The watchdog's host account is DEDICATED and OUT OF ROTATION — never share it with a work lane.** Owner rule, restated 2026-09-18 after the loop was walled: *"You're supposed to always be locked on a dedicated, isolated slot, with that account excluded from the rotation."* Two conditions, both required, checked EVERY tick:
  ...
  **And the choice is a LEASE, not a deed — re-evaluate it every time a 7d window resets (owner-endorsed 2026-09-18: "that is the right way to handle this").** The least-wanted account changes hour by hour, so the account you correctly took at 14:00 may be the fleet's best target by midnight. Each tick, ask: *is there now a cheaper host than the one I am sitting on?* A cheaper host is one whose loss costs the fleet less — lower 7d headroom, Fable already exhausted, no live lanes, no free families anyone is waiting on. When there is, hand the current host back (`cus enable <old>`) and move to the cheaper one using the self-rescue sequence below. Do it while you still have headroom, never at the wall, and never more than once per reset cycle — the point is to keep the good accounts working, not to churn your own mount. Worked example, 2026-09-18: rayi2 (7d 59%) was correct at 14:27Z because the cheaper-looking merkos (97%) and rayi1 (92%) were both hours from their weekly cap and would have walled the watchdog; once those reset, rayi2 goes back to the fleet.
...
- **NEVER place a lane on an account that is maxed on ANY axis — including the axis you were not looking at (2026-09-18, owner: *"swapping to a maxxed account isn't a great idea"*).** An account is a valid target only when **every** axis that lane can consume has real headroom: **5h**, **7d**, and — for a `premium`-pool lane — the **per-model (Fable) weekly**. The binding axis is whichever has the least room, and a target is only as good as that one. `cus status` prints all three; read all three before the move, on a FRESH poll.
  ...
  - **Rule of thumb:** a multi-pane lane needs roughly **25+ points** on the target's binding axis, or an axis that resets inside the next ~30 minutes. A single idle pane can go almost anywhere.
