...
>
> **THAT PATTERN IS INCOMPLETE — it misses the per-model (Fable) wall entirely (2026-09-22).** A per-model weekly cap does NOT render as the banner or the menu. It is an inline error in different words: **`You've reached your Fable limit. Run /usage-credits to continue or switch models with /model.`** None of `Usage limit`, `continuing automatically` or `Stop and wait` appear in it, so the sweep above returns nothing and the pane reads as healthy. **Use this instead:**
> ```bash
