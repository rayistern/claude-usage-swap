...
>
> **It compounds with the DO-NOT-INJECT default below — and the way to hold both is to CLASSIFY the idle pane, not to override the default (reconciled 2026-09-22 after the blind review of PR #243 found the first draft of this paragraph and the inject carve-out giving opposite verdicts on the incident's own panes).** An idle priority pane is a FINDING TO INVESTIGATE, never by itself an instruction to nudge. Read its last turns (the transcript, then the pane) and put it in one of three classes:
>   - **Stalled mid-task** — it stopped with work in hand (a plan half-executed, a next step it named itself and never ran, an agent that died under it) and only a message can resume it. This IS the carve-out below, clause (b): wake it, conversationally (peer-message rule), naming the step.
>   - **Done and waiting on the owner** — it finished what it had and is holding for an answer, a decision or an owner-only step. Do NOT nudge: the carve-out's "not deliberately done" and Keep-working's "genuinely done — leave it, just note it" both apply, and a nudge forces filler work and burns a turn. Instead RELAY: put its open questions, by pane name, in the tick report (and in the sheet's Notes if the owner reads there). The finding is reported every tick until the owner answers — it is never "fine"; it is just not yours to wake.
>   - **Unknown** — the pane does not tell you which. Send ONE signed conversational message asking exactly that (*"are you blocked on something I can supply, or waiting on the owner?"*), then act on the answer. That message is the investigation, not a nudge to work.
>
...
>
> **The owner's directive stands, and this is how it is carried out:** nothing is "parked" unless the owner says so, and waking panes / allocating usage is the watchdog's job — classification is how the watchdog does that job (it wakes the stalled, relays for the done, asks the unknown), not a reason to stop doing it.
>
...
**GREEN** iff: every protected pane is live; `cus sos` exit 0 (or the only SOS items are non-protected / benign — see SOS (d)); each protected pane's account is `5h% < ~90` **and** per-model-weekly `< ~95`; Status `ok` or `TOKEN_STALE`; nothing needlessly paused; no pane needed a nudge; **and every priority pane in the sheet is either moving or CLASSIFIED this tick (stalled → woken; done → its open questions relayed; unknown → asked)** — an unclassified idle priority pane is NOT green whatever the walls say (added 2026-09-22: the 30-hour "sweep clean" incident satisfied every other clause of this line). → Emit **one terse heartbeat line** and stop, nothing more:
...
**Do NOT nudge if:** actively working (spinner running); **genuinely done** / in a deliberate hold-or-daily-heartbeat mode (leave it, just note it — nudging forces filler work it chose not to do; a PRIORITY pane in this state is "done and waiting on the owner" under the first-question rule: relay its open questions, every tick, until the owner answers); or showing a **permission / yes-no prompt, tool-approval box, or ambiguous error** — never answer those on the human's behalf; leave it and escalate what it's asking. Max one nudge per pane per interval; if a pane ignores two consecutive nudges, stop and escalate.
