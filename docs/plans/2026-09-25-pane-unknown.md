...
One fixture, in the skill's tests.

## Checklist

- [x] Shim runs the newer reader when the vibeCoding checkout and the installed skill are different files. Evidence: `594e3de` (git commit time; mtime only if a candidate has no commit). Test: `test_newer_commit_wins_over_an_older_checkout_and_over_mtime` in `tests/test_pane_state.py` (`pytest tests/test_pane_state.py -q` → 13 passed, 2026-09-25).
...
- [x] Full suite, stated honestly. Local `pytest tests/ -q` on `f5b2194`: 879 passed, 1 failed (`test_flat_config_polls_everyone_each_interval`). That test passes on its own and fails the same way on `be98e0b` when the whole suite runs. GitHub Actions on `f5b2194` is green: `gh pr checks 251` → test 3.11, 3.12, 3.13 pass (run `36099819413`).
...
1. Shim resolution (`594e3de`): later git commit time when two reader files differ.
