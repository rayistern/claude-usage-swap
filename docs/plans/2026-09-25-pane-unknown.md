...
- [x] Shim runs the newer reader when the vibeCoding checkout and the installed skill are different files. Evidence: `2d2f53d` (one key `(has_commit, commit_time, mtime)`; a committed file never loses to mtime alone). Tests: `test_newer_commit_wins_over_an_older_checkout_and_over_mtime`, `test_untracked_copy_does_not_outrank_a_committed_reader` (`pytest tests/test_pane_state.py -q` → 17 passed, 2026-09-25).
...
- [x] Full suite, stated honestly. Local `pytest tests/ -q` on `2d2f53d`: 884 passed. The docs-and-fixtures commit's local run: 883 passed, 1 failed (`test_flat_config_polls_everyone_each_interval`). That test passes on its own and fails the same way on `be98e0b` when the whole suite runs. An earlier full run on `f5b2194` failed the same test (879 passed). GitHub Actions on `f5b2194` is green: `gh pr checks 251` → test 3.11, 3.12, 3.13 pass (run `36099819413`). CI on the later commits had not finished when this box was ticked.
...
1. Shim resolution (`2d2f53d`): one key `(has_commit, commit_time, mtime)`. A committed reader never loses to modification time alone. Two files print which one was chosen. One cold resolve measured about 40 ms; under load the same call measured 58–190 ms, so 40 ms is one measurement, not a bound.
