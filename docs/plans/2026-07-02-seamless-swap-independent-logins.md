...
> - **Still not built:** Phase 3b (lift the two #104 refusals for provably-independent families), Phase 4 (idle-expiry management, docs/README, flip the default). **Phase 0 experiments still not run** — but only the 2nd+-mount accounts need a login, so the burden is small (see `docs/plans/2026-07-02-phase0-login-runbook.md`).
