...
- **Single file**: `cus.py` is monolithic on purpose. Section headers (`# --- Foo ---`) separate concerns; navigate by them. It's grown to ~17k LOC; the single-file deployment story (below) is why it hasn't been split.
...
- **Single-file (~17k LOC) over multi-module package** — single-file is the deployment story
